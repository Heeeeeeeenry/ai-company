...
def classify_salience(entity: dict[str, Any], source_text: str) -> str:
    ...
    if kind == "code_symbol":
        return "event"  # code symbols: event-level salience (mutability-dependent)
...
def _extract_persons(text: str) -> list[dict[str, Any]]:
    ...
    # Compound surnames ("欧阳" / "司马" / "诸葛") must be matched BEFORE
    # the single-char surname regex, otherwise "欧" catches the first
    # character and "阳锋" becomes a false-positive single-character name.
    compound = getattr(cfg.entities, 'compound_surnames', [])
    if compound:
        _comp_pat = "|".join(re.escape(c) for c in sorted(compound, key=len, reverse=True))
        for m in re.finditer(
            rf"\b({_comp_pat})([一-鿿]{{1,2}})",
            text,
        ):
            surname_part = m.group(1)
            given = m.group(2)
            name = surname_part + given
            if any(name.endswith(s) for s in _NAME_STOP):
                continue
            out.append({"name": name, "kind": "person", "evidence": m.group(0)})
...
def extract_entities(text: str) -> list[dict[str, Any]]:
    ...
    # HanLP NER augmentation: fill gaps rule-based extraction misses
    # (person names, locations, organisations, etc.)
    from .hanlp_ner import enrich_entities as _enrich
    entities = _enrich(text, entities)
    # Code symbol extraction (L5): tree-sitter for Python code blocks
