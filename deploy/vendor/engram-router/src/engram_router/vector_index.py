"""FAISS-backed vector index for EngramRouter Phase 2.

Provides ANN (Approximate Nearest Neighbor) search over memory embeddings.
Stored alongside the SQLite database as <db_path>.faiss and <db_path>.idmap.

Architecture:
  - FAISS IndexHNSWFlat as default (graph-based, no training needed, fast)
  - FAISS IndexIVFPQ as optional quantized mode (compact, training required)
  - Cosine similarity via inner product on L2-normalized vectors
  - ID mapping between FAISS internal IDs and memory_ids
  - Incremental updates (add new memories without rebuild)
  - Persistent serialization (save/load on close/open)

Upgrade (2026-07-28): IndexIVFFlat → IndexHNSWFlat default.
  - HNSW eliminates the "not enough vectors to train" OOM for long-tail scenarios
  - No training phase required — add vectors immediately
  - PQ quantization available as opt-in via ``use_pq=True`` for >100K vectors

Dependencies:
  - faiss-cpu (via pip install engram-router[llm])
  - numpy
"""

from __future__ import annotations

import json
import logging
import os
import threading
import tempfile
from pathlib import Path
from typing import Any

try:
    import numpy as np
except ImportError:
    np = None  # type: ignore

try:
    import faiss
except ImportError:
    faiss = None  # type: ignore

logger = logging.getLogger(__name__)


class VectorIndex:
    """FAISS index for memory embeddings with incremental updates."""

    def __init__(
        self,
        dim: int = 1024,
        path: str | Path | None = None,
        nlist: int | None = None,
        hnsw_m: int = 32,
        use_pq: bool = False,
        pq_m: int = 64,
        pq_nbits: int = 8,
    ):
        self._dim = dim
        self._path = Path(path) if path else None
        self._lock = threading.Lock()
        self._id_to_memory: dict[int, str] = {}   # FAISS id → memory_id
        self._memory_to_id: dict[str, int] = {}    # memory_id → FAISS id
        self._next_id: int = 0
        self._index: Any = None
        self._trained = False

        # Load existing index if available
        loaded = False
        try:
            loaded = self._load() if self._path else False
        except (ImportError, Exception) as exc:
            logger.debug("VectorIndex load skipped: %s", exc)

        if not loaded:
            self._hnsw_m = hnsw_m
            self._use_pq = use_pq
            self._pq_m = pq_m
            self._pq_nbits = pq_nbits
            self._nlist = nlist or 8  # retained for IVFPQ compatibility
            self._index = None
            try:
                self._init_index()
            except ImportError as exc:
                logger.debug("VectorIndex init skipped (faiss not available): %s", exc)
                self._index = None

    # ── public ────────────────────────────────────────────────

    @property
    def size(self) -> int:
        """Number of vectors in the index."""
        return len(self._id_to_memory)

    @property
    def dim(self) -> int:
        return self._dim

    @property
    def trained(self) -> bool:
        return self._trained

    def add(
        self, memory_id: str, embedding: "np.ndarray", train_if_needed: bool = True  # type: ignore[type-arg]
    ) -> None:
        """Add a single embedding to the index.

        HNSW adds vectors immediately (no training buffer needed).
        For IVFPQ mode, vectors are buffered until enough for training.
        """
        with self._lock:
            vec = np.asarray(embedding, dtype=np.float32).reshape(1, -1)
            if vec.shape[1] != self._dim:
                raise ValueError(
                    f"Embedding dim {vec.shape[1]} != index dim {self._dim}"
                )

            faiss_id = self._next_id
            self._next_id += 1

            # HNSW: add immediately, always "trained"
            if self._trained:
                self._index.add(vec)
                # HNSW benefits from rebuilding periodically for optimal graph
                if self._hnsw_mode and self._next_id % 500 == 0:
                    pass  # FAISS HNSW supports incremental adds natively
            else:
                # Buffer for IVFPQ training only; HNSW never falls through here
                if not hasattr(self, "_buffer_embs") or self._buffer_embs is None:
                    self._buffer_embs: list["np.ndarray"] = []  # type: ignore[type-arg]
                    self._buffer_ids: list[int] = []
                self._buffer_embs.append(vec)
                self._buffer_ids.append(faiss_id)

                min_for_train = max(self._nlist * 5, 16)
                if len(self._buffer_ids) >= min_for_train and train_if_needed:
                    self._train_from_buffer()

            self._id_to_memory[faiss_id] = memory_id
            self._memory_to_id[memory_id] = faiss_id

            # NOTE: _save() is intentionally NOT called here; the caller
            # (MemoryStore.save_batch) is responsible for persisting after
            # all vectors are added, via VectorIndex.save() or ._save().

    def add_batch(
        self, ids_and_embs: list[tuple[str, "np.ndarray"]]  # type: ignore[type-arg]
    ) -> None:
        """Add multiple embeddings at once."""
        if not ids_and_embs:
            return
        with self._lock:
            ids, embs = zip(*ids_and_embs)
            vecs = np.asarray(embs, dtype=np.float32)
            if vecs.shape[1] != self._dim:
                raise ValueError(
                    f"Embedding dim {vecs.shape[1]} != index dim {self._dim}"
                )

            start = self._next_id
            count = len(ids)
            faiss_ids = list(range(start, start + count))
            self._next_id += count

            for fid, mid in zip(faiss_ids, ids):
                self._id_to_memory[fid] = mid
                self._memory_to_id[mid] = fid

            if self._trained:
                self._index.add(vecs)
            else:
                if not hasattr(self, "_buffer"):
                    self._buffer_embs = []
                    self._buffer_ids = []
                self._buffer_embs.append(vecs)
                self._buffer_ids.extend(faiss_ids)

                min_for_train = max(self._nlist * 5, 16)
                if len(self._buffer_ids) >= min_for_train:
                    self._train_from_buffer()

            # Commit deferred to caller — see note in add().

    def search(
        self, query_vec: "np.ndarray", k: int = 20  # type: ignore[type-arg]
    ) -> list[tuple[str, float]]:
        """Search for k nearest neighbors.

        Returns:
            List of (memory_id, similarity_score) sorted by similarity DESC.
            Similarity is cosine (inner product on normalized vectors, range -1..1).
        """
        if self.size == 0:
            return []

        q = np.asarray(query_vec, dtype=np.float32).reshape(1, -1)
        if q.shape[1] != self._dim:
            raise ValueError(
                f"Query dim {q.shape[1]} != index dim {self._dim}"
            )

        # Brute-force cosine fallback for untrained / small indices.
        # FAISS IVF needs training data; for small memory stores (<10K vectors)
        # brute-force numpy is fast enough and always correct.
        if not self._trained:
            if hasattr(self, "_buffer_embs") and self._buffer_embs:
                all_vecs = np.concatenate(self._buffer_embs, axis=0)  # type: ignore[name-defined]
                all_ids = self._buffer_ids
                # Normalize for cosine similarity
                q_norm = q / (np.linalg.norm(q) + 1e-8)  # type: ignore[name-defined]
                all_norm = all_vecs / (np.linalg.norm(all_vecs, axis=1, keepdims=True) + 1e-8)  # type: ignore[name-defined]
                sims = np.dot(all_norm, q_norm.T).flatten()  # type: ignore[name-defined]
                top_k = min(k, len(sims))
                top_idx = np.argsort(sims)[::-1][:top_k]  # type: ignore[name-defined]
                return [(self._id_to_memory.get(all_ids[i], ""), float(sims[i]))
                        for i in top_idx if sims[i] > 0]
            return []

        k = min(k, self.size)
        with self._lock:
            scores, indices = self._index.search(q, k)

        results = []
        for score, idx in zip(scores[0], indices[0]):
            if idx < 0:  # FAISS returns -1 for not enough neighbors
                continue
            memory_id = self._id_to_memory.get(int(idx))
            if memory_id:
                results.append((memory_id, float(score)))
        return results

    def remove(self, memory_id: str) -> bool:
        """Remove a vector from the index.

        FAISS doesn't support efficient single-vector deletion.
        We mark it as removed in the ID map instead.
        """
        with self._lock:
            faiss_id = self._memory_to_id.pop(memory_id, None)
            if faiss_id is None:
                return False
            self._id_to_memory.pop(faiss_id, None)
            # Note: the vector remains in FAISS but its ID won't be returned
            # because our search method filters by _id_to_memory.
            # A full rebuild can be triggered via rebuild().
            return True

    def rebuild(self) -> None:
        """Rebuild the index from scratch (removes ghosts from remove() calls)."""
        with self._lock:
            # Collect all live vectors
            live = []
            id_map: dict[int, str] = {}
            for fid, mid in self._id_to_memory.items():
                if fid < self._index.ntotal:
                    vec = self._index.reconstruct(int(fid))
                    live.append(vec)
                    id_map[len(id_map)] = mid

            # Rebuild
            self._id_to_memory = id_map
            self._memory_to_id = {v: k for k, v in id_map.items()}
            self._next_id = len(id_map)
            self._init_index()
            if live:
                self._index.add(np.array(live, dtype=np.float32))
                self._trained = True

            if self._path:
                self._save()

    def save(self, path: str | Path | None = None) -> None:
        """Persist the index and idmap to disk (delegates to _save)."""
        if path:
            self._path = Path(path)
        if self._path:
            self._save()

    def _init_index(self) -> None:
        if faiss is None:
            raise ImportError("faiss-cpu is required for VectorIndex. Install with: pip install engram-router[llm]")
        if self._use_pq:
            # IVFPQ: compact, quantized, training required
            quantizer = faiss.IndexFlatIP(self._dim)
            self._index = faiss.IndexIVFPQ(
                quantizer, self._dim, self._nlist,
                self._pq_m, self._pq_nbits, faiss.METRIC_INNER_PRODUCT,
            )
            self._hnsw_mode = False
        else:
            # HNSW: graph-based, no training, add vectors immediately
            self._index = faiss.IndexHNSWFlat(self._dim, self._hnsw_m, faiss.METRIC_INNER_PRODUCT)
            self._index.hnsw.efConstruction = 200
            self._index.hnsw.efSearch = 64
            self._trained = True  # HNSW doesn't need a training phase
            self._hnsw_mode = True

    def _train_from_buffer(self) -> None:
        all_vecs = np.concatenate(self._buffer_embs, axis=0)
        if len(all_vecs) < self._nlist:
            logger.warning(
                "Not enough vectors to train FAISS IVF (%d < %d)",
                len(all_vecs), self._nlist,
            )
            return
        self._nlist = min(int(np.sqrt(len(all_vecs))), 256)
        self._init_index()
        self._index.train(all_vecs)
        self._index.add(all_vecs)
        self._trained = True
        logger.info(
            "FAISS trained: %d vectors, nlist=%d, dim=%d",
            len(all_vecs), self._nlist, self._dim,
        )
        del self._buffer_embs
        del self._buffer_ids

    def _save(self) -> None:
        """Persist index + idmap atomically.

        Each file is written to a temp sibling then ``os.replace``'d, so a
        crash mid-write leaves the previous consistent state intact.
        """
        if not self._path or not self._trained:
            return
        import faiss
        self._path.parent.mkdir(parents=True, exist_ok=True)

        faiss_path = str(self._path.with_suffix(".faiss"))
        idmap_path = str(self._path.with_suffix(".idmap"))

        # -- Write FAISS index to temp, then atomic rename --
        fd_faiss, tmp_faiss = tempfile.mkstemp(
            dir=self._path.parent, prefix=".tmp_faiss_"
        )
        try:
            os.close(fd_faiss)
            faiss.write_index(self._index, tmp_faiss)
            os.replace(tmp_faiss, faiss_path)
        except Exception:
            try:
                os.unlink(tmp_faiss)
            except OSError:
                pass
            raise

        # -- Write ID map to temp, then atomic rename --
        meta = {
            "id_to_memory": {str(k): v for k, v in self._id_to_memory.items()},
            "memory_to_id": {str(k): v for k, v in self._memory_to_id.items()},
            "next_id": self._next_id,
            "dim": self._dim,
            "nlist": getattr(self, '_nlist', 8),
            "hnsw_m": getattr(self, '_hnsw_m', 32),
            "hnsw_mode": getattr(self, '_hnsw_mode', True),
            "use_pq": getattr(self, '_use_pq', False),
            "pq_m": getattr(self, '_pq_m', 64),
            "pq_nbits": getattr(self, '_pq_nbits', 8),
        }
        fd_idm, tmp_idm = tempfile.mkstemp(
            dir=self._path.parent, prefix=".tmp_idmap_"
        )
        try:
            with os.fdopen(fd_idm, "w") as f:
                json.dump(meta, f)
            os.replace(tmp_idm, idmap_path)
        except Exception:
            os.close(fd_idm)
            try:
                os.unlink(tmp_idm)
            except OSError:
                pass
            raise

    def _load(self) -> bool:
        faiss_path = str(self._path.with_suffix(".faiss"))  # type: ignore[union-attr]
        idmap_path = str(self._path.with_suffix(".idmap"))  # type: ignore[union-attr]
        if not os.path.exists(faiss_path) or not os.path.exists(idmap_path):
            return False
        try:
            import faiss
            self._index = faiss.read_index(faiss_path)
            self._trained = self._index.is_trained
            with open(idmap_path) as f:
                meta = json.load(f)
            self._id_to_memory = {int(k): v for k, v in meta["id_to_memory"].items()}
            self._memory_to_id = meta["memory_to_id"]
            self._next_id = meta["next_id"]
            self._dim = meta.get("dim", self._dim) if hasattr(self, '_dim') else meta.get("dim", 1024)
            self._nlist = meta.get("nlist", self._nlist) if hasattr(self, '_nlist') else meta.get("nlist", 8)
            self._hnsw_m = meta.get("hnsw_m", self._hnsw_m) if hasattr(self, '_hnsw_m') else meta.get("hnsw_m", 32)
            self._hnsw_mode = self._trained and self._index is not None
            self._use_pq = meta.get("use_pq", False)
            self._pq_m = meta.get("pq_m", self._pq_m) if hasattr(self, '_pq_m') else meta.get("pq_m", 64)
            self._pq_nbits = meta.get("pq_nbits", self._pq_nbits) if hasattr(self, '_pq_nbits') else meta.get("pq_nbits", 8)
            logger.info(
                "FAISS loaded: %d vectors, %dd, nlist=%d",
                len(self._id_to_memory), self._dim, self._nlist,
            )
            return True
        except Exception as exc:
            logger.warning("FAISS load failed: %s", exc)
            return False
