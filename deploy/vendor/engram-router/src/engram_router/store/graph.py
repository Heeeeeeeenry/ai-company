...
def index_edges(
    ...
    def _add_edge(src: str, dst: str, relation: str, confidence: float) -> None:
        ...
        conn.execute(
            "INSERT INTO edges (id, src_id, dst_id, relation, confidence, evidence_ref, namespace) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (edge_id, src, dst, relation, confidence, memory_id, namespace),
        )
    ...
    # Include valid_from timestamp so BFS can filter aged-out edges.
    _now = None  # lazy-import datetime only if needed
    for i in range(len(uniq)):
        for j in range(i + 1, len(uniq)):
            _add_edge(uniq[i]["id"], uniq[j]["id"], "CO_OCCURS_WITH", 0.4)
    # Set valid_from on all edges created in this batch
    try:
        ...
        pass  # non-critical
    if _now:
        conn.execute("UPDATE edges SET valid_from = ? WHERE evidence_ref = ? AND valid_from IS NULL",
                     (_now, memory_id))
    ...
    # ── Extended relationship types (L1) ──
    # IS_A: person → topic (e.g., 张三 IS_A 架构师)
    for ent in uniq:
        if ent["kind"] == "person":
            for other in uniq:
                if other["kind"] == "topic" and other["name"] in config.recall.occupation_topics:
                    _add_edge(ent["id"], other["id"], "IS_A", 0.9)

    # OWNS: person → object (heuristic: person near object in same memory)
    for ent in uniq:
        if ent["kind"] == "person":
            for other in uniq:
                if other["kind"] == "object" and other.get("name") != ent.get("name"):
                    _add_edge(ent["id"], other["id"], "OWNS", 0.7)

    # LOCATED_IN: company/location → location/time
    for ent in uniq:
        if ent["kind"] in ("company", "location"):
            for other in uniq:
                if other["kind"] == "location" and other.get("name") != ent.get("name"):
                    _add_edge(ent["id"], other["id"], "LOCATED_IN", 0.8)
                if other["kind"] == "time" and other.get("name") != ent.get("name"):
                    _add_edge(ent["id"], other["id"], "TEMPORAL_AT", 0.7)

    # CONTRADICTS: attribute conflict in same memory
    attrs = [e for e in uniq if e["kind"] == "attribute"]
    if len(attrs) >= 2:
        for i in range(len(attrs)):
            for j in range(i + 1, len(attrs)):
                _add_edge(attrs[i]["id"], attrs[j]["id"], "CONTRADICTS", 0.3)
...
def edge_expansion(
    ...
    _edge_cache: dict[str, list[dict[str, Any]]] = {}  # M15: per-src edge cache
    ...
    while queue:
        ...
        # M15: cache edges per src_eid to avoid re-querying if the same
        # entity is revisited by a stronger path later in BFS.
        # ``_edge_cache`` is populated lazily on first query for each eid.
        if src_eid in _edge_cache:
            _rows = _edge_cache[src_eid]
        else:
            _rows = []
