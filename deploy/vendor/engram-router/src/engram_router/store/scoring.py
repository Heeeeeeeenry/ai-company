...
class RecallWeights:
    ...
    **schema_version**: increment when fields are added, removed, or change
    default semantics. Downstream consumers can check this to detect
    incompatible configuration before it causes silent misbehavior.
    """

    # ── Schema version (API audit) ─────────────────────────────────────────
    schema_version: int = 1
    """Bump on field additions, removals, or default-semantics changes.
    Consumers should check this against their expected version at startup."""
    ...
    person_isolation_scope_weight: float = 0.25
    """Multiplier for person-isolation narrowing: when query names a person,
    memories without that person have their score multiplied by this factor.
    0.25 means a 75% penalty for no-person-match memories. This closes the
    gap where memories with no person entity (e.g., '她特别喜欢猫') escaped
    person checks entirely."""
    person_isolation_subtract: float = 1.0
    """Subtractive penalty for person-isolation: when query names a person,
    memories with a DIFFERENT person entity get this subtracted.
    Memories with NO person entity at all get a milder penalty since they
    may be coreference (pronoun-only: "他/她" linked to a prior sentence)."""
    person_isolation_noentity_weight: float = 0.5
    """Multiplier for memories with NO person entity (likely coreference).
    More generous than the 0.25 used for clear conflicts, to avoid penalizing
    pronoun-only sentences that refer back to the query's target person."""
    person_isolation_noentity_subtract: float = 0.2
    """Subtractive penalty for no-person-entity memories (less aggressive
    than the full person_isolation_subtract, since these are likely pronouns)."""
    person_match_boost: float = 2.0
    entity_tie_break_bonus: float = 0.01

    # ── LLM intent classifier (L1.3) ───────────────────────────────────────
    intent_classifier_enabled: bool = False
    """Enable LLM-based intent classification in recall().
    When True and the LLM is available, the intent classifier replaces
    the regex-based asks_* functions with soft probability outputs.
    Default is False to avoid latency regression."""
    ...
    """Enable HyDE (Hypothetical Document Embeddings). On by default since
    2026-07-24 after hardening negative-pattern detection. Falls back
    gracefully when LLM key / embedding engine / vector index is missing."""
    ...
    # ── Three-channel retrieval (L1.1) ──
    channel_top_n: int = 50
    """Number of candidates each channel retrieves before fusion."""
    channel_fts_weight: float = 0.4
    """Weight for the FTS+entity+edge channel in fusion."""
    channel_vector_weight: float = 0.6
    """Weight for the vector bi-encoder channel in fusion."""
    channel_hyde_weight: float = 0.25
    """Weight for the HyDE channel in fusion (lower = less influence)."""
    fusion_method: str = "rrf"
    """Fusion method: 'rrf', 'isf', 'weighted_sum', 'learned_isf'."""
    fusion_k: int = 60
    """RRF damping constant k."""

    # ── Multi-query recall (Phase 3 of rerank_and_hyde.md) ──────────────
    multi_query_enabled: bool = False
    """Enable multi-query expansion: generate 3-5 variants of the query,
    run independent recall per variant, and RRF-fuse the results.
    Off by default until verified."""
    multi_query_num_variants: int = 3
    """Number of query variants to generate (3-5 recommended)."""
    multi_query_fusion_weight: float = 0.5
    """Weight of multi-query result lists vs primary query in fusion.
    Lower values keep multi-query as a supplement, not the primary signal."""

    # ── Recall snapshots (L4.3 observability) ──────────────────────────
    sampling_rate: float = 0.01
    """Fraction of recall() calls to snapshot (0.0 disables, 1.0 = all).
    Snapshots record query/result_ids/trace for offline replay and debugging.
    Default 1% avoids storage bloat in production while still capturing samples."""

    # ── Recall deadline (N7 SRE) ────────────────────────────────────────
    recall_deadline_ms: int = 5000
    """End-to-end wall-clock budget for recall() in milliseconds.

    When the deadline is exceeded inside _recall_impl(), expensive phases
    (HyDE LLM call, CE rerank, ColBERT, LLM-rerank) are skipped and the
    best results so far are returned.  The cheaper phases (FTS token match,
    entity extraction, edge expansion) always complete.

    Set to 0 to disable the deadline (no budget limit)."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.channel_top_n < 1:
            raise ValueError(f"channel_top_n must be >= 1, got {self.channel_top_n}")
        if self.channel_fts_weight < 0:
            raise ValueError(f"channel_fts_weight must be >= 0, got {self.channel_fts_weight}")
        if self.channel_vector_weight < 0:
            raise ValueError(f"channel_vector_weight must be >= 0, got {self.channel_vector_weight}")
        if self.channel_hyde_weight < 0:
            raise ValueError(f"channel_hyde_weight must be >= 0, got {self.channel_hyde_weight}")
        if self.fusion_method not in ("rrf", "isf", "weighted_sum", "learned_isf"):
            raise ValueError(f"fusion_method must be one of rrf/isf/weighted_sum/learned_isf, got {self.fusion_method}")
        if self.fusion_k < 0:
            raise ValueError(f"fusion_k must be >= 0, got {self.fusion_k}")
        if self.multi_query_num_variants < 1:
            raise ValueError(f"multi_query_num_variants must be >= 1, got {self.multi_query_num_variants}")
        if not (0.0 <= self.multi_query_fusion_weight <= 5.0):
            raise ValueError(f"multi_query_fusion_weight must be in [0, 5], got {self.multi_query_fusion_weight}")
        if not (0.0 <= self.sampling_rate <= 1.0):
            raise ValueError(f"sampling_rate must be in [0, 1], got {self.sampling_rate}")
...
def _default_weights() -> RecallWeights:
    ...
    return RecallWeights(
        ...
        hyde_rrf_weight=getattr(c, 'hyde_rrf_weight', 0.25),
        hyde_skip_vector_penalty=getattr(c, 'hyde_skip_vector_penalty', 0.8),
        hyde_only_penalty=getattr(c, 'hyde_only_penalty', 0.7),
    )
